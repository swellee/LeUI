package layouts
{
	...
	public class BoxLayout extends BasicLayout
	{
		...
		{
			var matrixContainer:IMatrixContainer=contianer as IMatrixContainer;
			if(!matrixContainer)
			{
				throw new LTypeError("BoxLayout 's para must be IMatrixContainer");
			}
			
			var eles:Vector.<IlayoutElement>=matrixContainer.layoutElements;
			var eleCount:int=eles.length;
			var ele:IlayoutElement;
			var noneScaleEleCount:int=0;
			var noneScaleEleSizeSum:int=0;
			var isHorizonLayout:Boolean=matrixContainer.direction==UiConst.HORIZONTAL;
			var margin:int=isHorizonLayout?matrixContainer.hGap:matrixContainer.vGap;
			//计算不可缩放的子对象的数量和总尺寸
			...
			var containerBounds:Rectangle=matrixContainer.bounds;
			...
			for (var j:int = 0; j < eleCount; j++) 
			{
				...
				if(isHorizonLayout)//如果是横向布局
				{
					...
					{
						LGeom.centerInCoordY(ele as DisplayObject,matrixContainer as DisplayObjectContainer);
					}
				...
				else
				{
					...
					{
						LGeom.centerInCoordX(ele as DisplayObject,matrixContainer as DisplayObjectContainer);
					}
